...
To learn more, see [SIP overview](https://docs.livekit.io/telephony.md) and the [SIP API](https://docs.livekit.io/reference/telephony/sip-api.md) reference.

## Dispatch to an agent

To have an agent answer each inbound call, use an individual dispatch rule so each caller is placed in a dedicated room, and add a `roomConfig` option to the rule to dispatch your agent to those rooms. See [Agent dispatch](#agent-dispatch) for `roomConfig` parameters.
...
This document was rendered at 2026-04-12T10:16:36.022Z.
